...
Version  : 0.2.0
Release  : 1
URL      : https://cran.r-project.org/src/contrib/hardhat_0.2.0.tar.gz
Source0  : https://cran.r-project.org/src/contrib/hardhat_0.2.0.tar.gz
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1645307408
...
R CMD INSTALL --install-tests --built-timestamp=${SOURCE_DATE_EPOCH} --build  -l %{buildroot}/usr/lib64/R/library hardhat
for i in `find %{buildroot}/usr/lib64/R/ -name "*.so"`; do mv $i $i.avx2 ; mv $i.avx2 ~/.stash/; done
echo "CFLAGS = $CFLAGS -march=x86-64-v4 -ftree-vectorize  -mno-vzeroupper " > ~/.R/Makevars
echo "FFLAGS = $FFLAGS -march=x86-64-v4 -ftree-vectorize  -mno-vzeroupper " >> ~/.R/Makevars
echo "CXXFLAGS = $CXXFLAGS -march=x86-64-v4 -ftree-vectorize -mno-vzeroupper  " >> ~/.R/Makevars
R CMD INSTALL --preclean --install-tests --no-test-load --built-timestamp=${SOURCE_DATE_EPOCH} --build  -l %{buildroot}/usr/lib64/R/library hardhat
for i in `find %{buildroot}/usr/lib64/R/ -name "*.so"`; do mv $i $i.avx512 ; mv $i.avx512 ~/.stash/; done
echo "CFLAGS = $CFLAGS -ftree-vectorize " > ~/.R/Makevars
echo "FFLAGS = $FFLAGS -ftree-vectorize " >> ~/.R/Makevars
echo "CXXFLAGS = $CXXFLAGS -ftree-vectorize " >> ~/.R/Makevars
R CMD INSTALL --preclean --install-tests --built-timestamp=${SOURCE_DATE_EPOCH} --build  -l %{buildroot}/usr/lib64/R/library hardhat
...
R CMD check --no-manual --no-examples --no-codoc hardhat || :
...
/usr/lib64/R/library/hardhat/tests/testthat/out/print-blueprint-default-1.txt
/usr/lib64/R/library/hardhat/tests/testthat/out/print-blueprint-formula-1.txt
/usr/lib64/R/library/hardhat/tests/testthat/out/print-blueprint-formula-2.txt
/usr/lib64/R/library/hardhat/tests/testthat/out/print-blueprint-recipe-1.txt
/usr/lib64/R/library/hardhat/tests/testthat/out/print-hardhat_model-1.txt
/usr/lib64/R/library/hardhat/tests/testthat/out/print-hardhat_model-2.txt
/usr/lib64/R/library/hardhat/tests/testthat/out/print-hardhat_model-3.txt
